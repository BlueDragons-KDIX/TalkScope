# test-spec-007: Term category / score（難易度ランク）廃止
...
- 手動確認: 説明ウィンドウ・文字起こしホバー・バブルツールチップに初級/中級/上級および `Score.N` が出ないこと。
- 関連 ADR: `adr-009.md`
- テスト仕様番号は `develop/v0` 既存の `test-spec-006`（ウィンドウ個別設定 UI）と重複しないよう `007` とした。
