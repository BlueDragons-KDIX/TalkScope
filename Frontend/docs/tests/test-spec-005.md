...
2026-05-20（コピー形式の拡張は `test-spec-026.md` / order-014 を参照）

> **注記（2026-05-29）:** テストメニューのコピーは `LayoutNode` メソッド形式から **プレゼン設定 JSON**（コメントなし）に変更された。レイアウトのみの検証は本 spec、フルスナップショットは `test-spec-026.md` を参照。
...
| 5 | スクリプトテンプレートを標準一覧として表示する | 手動 | `バブル重視` / `ランキング重視` / `フルカスタム` がレイアウトボタンに表示される（order-015 以降） |
