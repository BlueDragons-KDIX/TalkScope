...
def test_create_and_read():
    ...
    # 前回のテストデータが残っていたら削除
    old_entry = read_dictionary_by_word(word)
    if old_entry:
    ...
    idx = create_dictionary(word, description)
    create_time = time.perf_counter() - start
    print(f"✅ ワード追加成功: id={idx}  ⏱️ {create_time:.3f}秒")

    # 取得
    start = time.perf_counter()
    entry = read_dictionary_by_word(word)
    read_time = time.perf_counter() - start
    assert entry is not None, f"❌ '{word}' が見つからない"
    assert entry.id == idx, f"❌ id が一致しない: expected={idx}, got={entry.id}"
    assert entry.word == word, f"❌ word が一致しない: {entry.word}"
    assert entry.description == description, f"❌ description が一致しない: {entry.description}"
    print(f"✅ ワード取得成功: id={entry.id}  ⏱️ {read_time:.3f}秒")
...
def test_delete(idx: int):
    ...
    start = time.perf_counter()
    entry = read_dictionary_by_word("test_apple")
    read_time = time.perf_counter() - start

    assert entry is None, f"❌ 削除されていない: id={entry.id}"
    print(f"✅ ワード削除成功: id={idx}  ⏱️ {delete_time:.3f}秒")
